...
contract TriathonAuction is Ownable, IERC721Receiver {
    ...
    function bid(uint poolId) external isPoolOpen(poolId) {
        ...
        if (bidderNumber != 0) {   // update lastBidder's reward.
            ...
            bidderReward[poolId][pst.lastBidder] = bidderReward[poolId][pst.lastBidder].add(lastBidderReward);
        }
    ...
    function getBidderReward(uint poolId, address bidder) external view isPoolExist(poolId) returns (uint ret) {
        PoolStatus memory pst = poolsStatus[poolId];
        if (pst.status == POOL_OPEN || pst.status == POOL_CANCELED || pst.status == POOL_PASSED || pst.status == POOL_RETRIEVALED) {
            ret = 0;
        } else if (pst.status == POOL_BIDDING) {
            ret = bidderReward[poolId][bidder];
        } else if (pst.status == POOL_DATED || pst.status == POOL_CLOSED) {
            if (bidder == pst.lastBidder) {
                ret = pst.reservedReward.add(bidderReward[poolId][bidder]);
            } else {
                ret = bidderReward[poolId][bidder];
            }
        }
